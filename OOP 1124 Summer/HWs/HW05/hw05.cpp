...
template <struct T>
void listInsertHead(T entry, Node*& headPtr) {
	headPtr = new Node(entry, headPtr);
}

template <struct T>
Node* createList(Node*& headPtr){
	Node* head = new Node(headPtr);
	T newdata;
	while(newdata != "DONE"){
		cout << "Enter a value, type DONE when finished: ";
		cin >> newdata;
		listInsertHead(T newdata, head);
	}
	return head;
}
